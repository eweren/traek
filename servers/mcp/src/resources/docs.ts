...
function formatComponentAsMarkdown(name: string): string {
	...
	if (doc.methods && doc.methods.length > 0) {
		...
		for (const m of doc.methods) {
			lines.push(`### \`${m.name}\``, `\`\`\`typescript`, m.signature, `\`\`\``, ``, m.description, ``)
		}
	}
...
export const resourceHandlers = [
	{
		...
		handler: async (uri: URL, params: { name: string }) => {
...

	{
		...
		handler: async (uri: URL, params: { name: string }) => {
			const guide = getGuide(params.name)
			const text = guide
				? guide.content
				: `Guide "${params.name}" not found. Available: getting-started, openai-streaming, custom-nodes, persistence, theming, sveltekit-setup`
...

	{
		...
		handler: async (uri: URL, params: { name: string }) => {
			const snippet = getSnippet(params.name)
			const text = snippet
				? `# ${snippet.title}\n\n${snippet.description}\n\n\`\`\`${snippet.language}\n${snippet.code}\n\`\`\``
				: `Snippet "${params.name}" not found. Run list_snippets to see all available IDs.`
