...
import type { LayoutInput, NodePosition, LayoutMode } from './types';
...
// ── Tree Horizontal ───────────────────────────────────────────────────────────
// Rotates 90° clockwise: parent left, children to the right

function treeHorizontal(input: LayoutInput): NodePosition[] {
	const positions: NodePosition[] = [];
	const { nodeWidthGrid, nodeHGrid, gapXGrid, gapYGrid } = input.config;

	// For horizontal layout, we swap x/y roles:
	// depth → x axis, sibling position → y axis
...
// ── Force-Directed ────────────────────────────────────────────────────────────
// Simple Fruchterman-Reingold spring simulation (15 iterations for quick layout)
...
function forceDirected(input: LayoutInput): NodePosition[] {
	...
	// Start from current positions as initial state
	const pos = new Map<string, { x: number; y: number }>(
		nodes.map((n) => [n.id, { x: n.metadata?.x ?? 0, y: n.metadata?.y ?? 0 }])
	);

	// Build edge set (parent→child connections)
...
function timeline(input: LayoutInput): NodePosition[] {
	...
	// Position: x = depth, y = index within depth
