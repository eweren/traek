...
export const darkTheme: TraekTheme = {
	...
	typography: {
		...
		sizes: { xs: '10px', sm: '12px', base: '14px', lg: '16px', xl: '20px', '2xl': '28px' },
...
export function createCustomTheme(baseTheme: TraekTheme, accentColor: string): TraekTheme {
	const v = generateAccentVariations(accentColor);
	const t: TraekTheme = JSON.parse(JSON.stringify(baseTheme));
	t.colors.nodeActiveBorder = v.base;
	t.colors.nodeActiveGlow = v.alpha15;
	t.colors.nodeUserBorderTop = v.base;
	t.colors.connectionHighlight = v.base;
	t.colors.inputButtonBg = v.base;
	t.colors.inputDot = v.base;
	t.colors.thoughtPanelBorderActive = v.base;
	t.colors.thoughtPanelGlow = v.alpha15;
	t.colors.thoughtTagCyan = v.base;
	t.colors.thoughtBadgeCyan = v.light;
	t.colors.overlayPillBg = v.base;
	t.colors.overlayPillShadow = v.alpha20;
	return t;
}
